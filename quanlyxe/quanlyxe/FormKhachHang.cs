    ...
    public partial class FormKhachHang : Form
    {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    connection.Open();
                    string query = "INSERT INTO KhachHang (MaKhachHang, TenKhachHang, DiaChi, DienThoai) " +
                                   "VALUES (@MaKhachHang, @TenKhachHang, @DiaChi, @DienThoai)";

                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        command.Parameters.AddWithValue("@MaKhachHang", int.Parse(textBox1.Text)); // Giả sử textBoxMaKhachHang là TextBox cho mã nhân viên
