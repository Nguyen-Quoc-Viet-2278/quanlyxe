    ...
    public partial class FormNhanVien : Form
    {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    connection.Open();
                    string query = "INSERT INTO NhanVien (MaNhanVien, TenNhanVien, GioiTinh, NamSinh, DiaChi, DienThoai, Email) " +
                                   "VALUES (@MaNhanVien, @TenNhanVien, @GioiTinh, @NamSinh, @DiaChi, @DienThoai, @Email)";

                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        command.Parameters.AddWithValue("@MaNhanVien", int.Parse(textBox1.Text)); // Giả sử textBoxMaNhanVien là TextBox cho mã nhân viên
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    try
                    {
                        ...
                        string query = "UPDATE NhanVien SET TenNhanVien = @TenNhanVien, NamSinh = @NamSinh, " +
                                       "GioiTinh = @GioiTinh, DiaChi = @DiaChi, DienThoai = @DienThoai, Email = @Email " +
